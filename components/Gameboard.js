...
export default function Gameboard() {
  ...
  const [bombs, setBombs] = useState(NBR_OF_BOMBS);
  ...
  function startGame() {
    // this also works as a new game function, therefore it resets everything to default states first
    initialBoard = [
      START, START, START, START, START,
      START, START, START, START, START,
      START, START, START, START, START,
  ...
  function drawShips() {
    // chooses three ships locations. do..while keeps checking if number already exists in the array and picks a new one if it does
    const shipPositions = [];
    for (let i = 0; i < NBR_OF_SHIPS; i++) {
      ...
      while (shipPositions.indexOf(randomNumber) !== -1);
      shipPositions.push(randomNumber);
    }
    setShipArray(shipPositions);
    //console.log(shipPositions);
  }

  function drawItem(number) {
    if (start) {
      if (board[number] === START && checkGame() === "") {
        if (shipArray.indexOf(number) !== -1) {
          board[number] = CIRCLE;
          setHits(hits + 1);
          setShips(ships - 1);
        }
    ...
    // needed to add the checkGame check here because otherwise this status would appear if you clicked a plus symbol after game had ended
    else if (checkGame() === "") {
  ...
  function checkGame() {
    ...
    if (bombs === 0 && ships !== 0) {
      setStatus('Game over. Out of bombs.');
      setStart(false);
      clearInterval(timerRef.current);
    }
    else if (ships === 0) {
      setStatus('You sank all the ships!');
      setStart(false);
      clearInterval(timerRef.current);
    }
    else if (seconds === 0) {
      setStatus('Game over. Ran out of time.');
      setStart(false);
      clearInterval(timerRef.current);
    }
    else {
  ...
  return (
    <View style={styles.gameboard}>
      ...
      <View style={styles.flex}>
        ...
        <Text style={styles.gameinfo}>Bombs: {bombs}</Text>
